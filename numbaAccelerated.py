...
def movePeriodically(xs, wheres, left, right):
    """
...
def fromAtoB(ia, ib, xsa, ysa, vxsa, vysa, wheresa, lastColla,cola, xsb, ysb, vxsb, vysb, wheresb, lastCollb,colb):
    # swap
...
@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def moveToSwap(xsa, ysa, vxsa, vysa, wheresa, lastColla,cola, xsb, ysb, vxsb, vysb, wheresb, lastCollb, colb, xLim, kindOfLim):
    """
    ...
    for i in range(len(xsa)):
        if wheresa[i] != 0 and (xsa[i] > xLim) == kindOfLim:
            fromAtoB(i, count, xsa, ysa, vxsa, vysa, wheresa, lastColla,cola, xsb, ysb, vxsb, vysb, wheresb, lastCollb, colb)
            count += 1
...
@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def moveSwapToNeighbor(xsa, ysa, vxsa, vysa, wheresa, lastColla, cola, xsb, ysb, vxsb, vysb, wheresb, lastCollb, colb, amount, ymax):
    """
...
@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def roll1(i, ar1, ar2, ar3, ar4, ar5, ar6, ar7):
    """
    set values formerly at index i-1 to index i for 7 arrays
...
@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def sortCell(xs, ys, vxs, vys, wheres, lastColls, colors):
    """
    ...
    for i in range(1, len(ys)):
        ...
        swaps = 0
        j = i - 1
        x, y, vx, vy, where, lastColl,col = xs[i], ys[i], vxs[i], vys[i], wheres[i], lastColls[i],colors[i]
...
"""
TODO 
For the moment : all particle/wall collisions are handled as fast as possible, but in disorder
For the future : it would be worth trying to sort the collisions by time and effect them in order !!
"""

# don't set nogil = True so that only one thread at a time modifies wall location
@jit(nopython=True, cache=True, fastmath=True)#, nogil=True)
def movingWallInteraction(xs, vxs, vys, wheres,lastColls,colors, x, v, dt, mp, m,time):
    """
    ...
    :param colors: additionnal array
...
@jit(nopython=True, cache=True, fastmath=True)#, nogil=True)
def movingWallInteractionOLD(xs, vxs, vys, wheres,lastColls,colors, x, v, dt, mp, m,time):
    """
    update coordinates and velocities of particles interacting left wall (if exists)
    The exact time (if wall velocity is constant) is first computed ; then the particle is correctly bounced back
    ...
    return fpl, fpr  # should return now x and v of wall


@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def staticWallInteractionLeft(xs, vxs, vys, wheres,lastColls,colors, left, dt, m,time):
    """
...
@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def staticWallInteractionRight(xs, vxs, vys, wheres, lastColls,colors, right, dt, m,time):
    """
        update coordinates and velocities of particles interacting left wall (if exists)
...
@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def staticWallInterractionDown(ys, vxs, vys, wheres,lastColls,colors, width, dt, m, vStar,time):
    """
...
@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def staticWallInterractionUp(ys, vxs, vys, wheres,lastColls,colors, width, dt, m, vStar,time):
    """
    update coordinates and velocities of particles interacting with solid walls (which are horizontal)
    Compute the forces applied to both walls
...
@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def computeMacroEcs(vxs, vys, wheres, m):
    """
        compute kinetic energy left and right to wall
        :param vxs: x velocities
        :param vys: y velocities
...
@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def detectCollisionsAtInterface(indices, nindices, xs, ys, vxs, vys, wheres, lastColls,colors, nxs, nys, nvxs, nvys, nwheres,
                                nlastColls,ncolors, dt, d, time):
    """
...
@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def detectAllCollisions(xs, ys, vxs, vys, wheres, lastColls,colors, indicesLeft, indicesRight, dt, d, histo, coloringPolicy,
                        left, right, time):
    """
    This method update particle positions and velocities taking into account elastic collisions
    ...
    for i in range(ln - 1):  # last particle can't collide to anyone else
        ...
        for j in range(i + 1, ln):
            if 1. * w * wheres[j] <= 0:  # different side (<0) or one dead particle (==0)
...
@jit(nopython=True, cache=True, fastmath=True, nogil=True)
def computeColorRatio(wheres,colors,threshold):
    count = 0
    total = 0
