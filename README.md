...
It targets the simulation of non equilibrium flows, and interractions with boundaries. It Relies heavily on Numpy and Numba to compete with code written in faster langages. Finally, DAYS is an out of order acronym for Yet Another Disk Simulation, as the code rely on an "out of order" approach for the treatment of the collisions.



## In short

This project aim at simulating from 1k up to 100M of particles (2D disks) and their ellastic collisions, in ordrer to retrieve some well known results in thermodynamics :
- mean free path
- equation of state
...
Then some other files, in dedicated directories, permits to launch specific simulations, for which results can be easely displayed by python scripts, located in the same directories, using matplotlib.
