...
# 64 k
#vxs1.append(np.array([5.44,14.16,22.17,28.87,34.58,39.68,44.57,48.01,51.37,53.7,55.87,56.61]))
# 128 k
vxs1.append(np.array([5.99,14.77,22.67,30.05,36.08,41.8,46.22,50.03,53.33,55.4,56.58,57.8]))
# 256 k
vxs1.append(np.array([6.24,15.47,23.79,31.03,37.46,42.83,47.71,51.73,54.89,56.96,58.46,59.11]))
# 512 k
vxs1.append(np.array([5.97,15.2,23.31,30.55,36.69,42.26,47.33,51.47,54.59,56.77,58.65,59.08]))
...
#64k
#vxs2.append(np.array([4.92,11.69,17.97,22.89,27.4,31.61,35.49,38.37,40.24,41.49,42.25,42.24]))
#128k
vxs2.append(np.array([4.79,11.35,17.01,21.73,26.2,30.09,33.53,36.25,38.47,40.19,41.49,42.17]))
#256k
vxs2.append(np.array([4.79,11.13,16.77,21.81,26.67,30.68,34.14,37.1,39.23,40.92,42.29,42.85]))
#512k
vxs2.append(np.array([4.58,11.32,17.06,22.1,26.54,30.68,34.42,37.49,39.78,41.4,42.12,42.46]))
...
'''
# pour 64k particules
#vxs3.append(np.array([4.09,9.18,13.21,16.87,20.24,22.46,24.89,27.56,29.44,30.18,30.41,30.27]))

# pour 128k particules
vxs3.append(np.array([3.55,8.03,12.32,15.96,19.22,22.17,24.65,26.49,28.09,29.43,30.27,30.56]))

# pour 256k particules
vxs3.append(np.array([3.94,8.86,13.11,16.66,19.91,22.7,25.19,27.27,29.04,30.06,31.16,31.68]))

# pour 512k particules
vxs3.append(np.array([3.8,8.63,12.8,16.45,19.64,22.57,25.06,27.03,28.67,29.73,30.43,30.81]))
...
"""
l4 = 4.0e-3
T4 =np.average([312,313,313])
vxs4 =[]


# pour 128k particules
vxs4.append(np.array([2.66,5.15,7.37,9.12,10.74,12.03,13.28,14.26,15.06,15.57,15.66,15.76]))

# pour 512k particules
vxs4.append(np.array([2.52,4.99,7.28,9.08,10.59,12.14,13.39,14.45,15.32,15.98,16.42,16.69]))

# pour 1024k particules
vxs4.append(np.array([2.6,5.27,7.2,8.73,10.23,11.62,13.01,13.46,14.48,15.17,15.84,15.84]))
...
def addProfileToPlot(vxs,T,l,color):
    ...
    print("eta simu/modèle", str(round(np.average(etas),4))+" +/-"+ str(round(np.std(etas)/np.sqrt(len(etas)),4)), round(eta0(l, T, Ns),4))
