...
class WallVelocity:
    ...
    def update(self,d:Domain):
        if d.wall is not None:
            self.times.append(d.csts["time"]*1000)
...
class Temperatures:
    ...
    def update(self,d:Domain):
        ...
        dpg.set_axis_limits(self.tag + "vx_axis", 0, self.times[-1])  # time in ms
        if d.wall is not None:
            self.tempLeft.append(d.computeKineticEnergyLeftSide() / d.countLeft() / d.csts["kbs"])
            self.tempRight.append(d.computeKineticEnergyRightSide() / d.countRight() / d.csts["kbs"])
            ...
            self.tempRight.append(d.computeKineticEnergy() / d.count() / d.csts["kbs"])
...
class Laplaces:
    ...
    def update(self,d:Domain):
        if d.wall is not None:
            if self.first :
                self.first = False
                self.initLeft = d.computeKineticEnergyLeftSide() / d.countLeft() / d.csts["kbs"] * d.wall.location()
                self.initRight = d.computeKineticEnergyRightSide() / d.countRight() / d.csts["kbs"] * (d.csts["width"]- d.wall.location())

            self.times.append(d.csts["time"]*1000)
            self.laplaceLeft.append(d.computeKineticEnergyLeftSide() / d.countLeft() / d.csts["kbs"] * d.wall.location() / self.initLeft)
            self.laplaceRight.append(d.computeKineticEnergyRightSide() / d.countRight() / d.csts["kbs"] * (d.csts["width"]- d.wall.location()) / self.initRight)
            dpg.set_axis_limits(self.tag+"vx_axis", 0, self.times[-1]) # time in ms
