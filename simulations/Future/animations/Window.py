...
from numbaAcc.helping import twoArraysToTwo
...
class Window:
    ...
    def __init__(self, nPart, P, T, L, H, ls, nbCells=1, ratios=None, effectiveTemps=None, resX=1024, resY=1024,
        ...
        domain = Domain(4, T, self.X, self.Y, P, nPart, ls, drOverLs=0.02, maxWorkers=1, periodic=periodic, effectiveTemps= effectiveTemps, ratios = ratios)
        self.wallTarget = 0.5
        self.resX = resX
        self.resY = resY
    ...
    def run(self):
        dpg.create_viewport(title='Particle simulation', width=self.resX, height=self.resY)
        dpg.setup_dearpygui()
        dpg.show_viewport()
    ...
    def getRadius(self):
        radius = (self.domain.csts["ds"] / self.domain.csts["length"] * self.resX / 2 *1.05)
        return radius
