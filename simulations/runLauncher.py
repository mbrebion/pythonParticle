...
def outputFileName(dd):
    return "./runs/"+str(int(dd["nPart"] / 1000)) + "_" + str(dd["nbDomain"]) + "_" + str(dd["vstarov"]) + ".txt"
...
def outputCriterion(d, dd):
    """
    decide whether data should be output
    :param d: domain
    :param d: data dict
    :return: True if data is to be output ; False else
    """

    return ComputedConstants.it % 500 == 0
...
def runCriterion(d, dd):
    ...
    """
    return d.wall.location() > 4 * dd["X"] / 10
